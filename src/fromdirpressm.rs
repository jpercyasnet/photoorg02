...
//        .unwrap();
     ...
     if folder == None {
         ...
         if errcda != 0 {
             ...
             new_dirlist = newlist;
             if sizetxt.len() == 0 { 
                 errstring = "********* Icon Size has no value **********".to_string();
                 errcode = 2;
             } else {
                 let icon_int: i32 = sizetxt.parse().unwrap_or(-99);
                 if icon_int > 0 {
                     if (icon_int < 50) | (icon_int > 255) {
                         errstring = "********* Icon Size not between 50 and 255 **********".to_string();
                         errcode = 3;
                     } else {
                         icon_int1 = icon_int;
                     ...
                     errstring = "********* Icon Size is not an integer **********".to_string();
                     errcode = 4;
                 } else {
                     errstring = "********* Icon Size not positive integer **********".to_string();
                     errcode = 5;
                 }
             }
         }
